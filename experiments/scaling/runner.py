...
class ScalingRunner:
    ...
    def __init__(
        ...
        # Results are routed by the pyAgrum major version actually running
        # (e.g. agrum3/), so runs from different stacks never mix.
        agrum_tag = f"agrum{pyagrum.__version__.split('.')[0]}"
        self.results_dir = (
            Path(__file__).parent / agrum_tag / f"{timestamp}_{generator_name}"
        )
